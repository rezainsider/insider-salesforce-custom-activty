...
(function(root, factory) {
    ...
    Session = Postmonger.Session = function() {
        ...
        for (i = 0,
            ...
            if (connection) {
                for (j = 0,
                         ln = connections.length; j < ln; j++) {
                    if (connections[j].connect === connection.connect && connections[j].from === connection.from && connections[j].to === connection.to) {
                        connection = null;
                        break;
                    }
                }
        ...
        postMessageListener = function(event) {
            ...
            var k, len;
            for (k = 0,
                     len = connections.length; k < len; k++) {
